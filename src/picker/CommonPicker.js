...
import WheelCurvedPicker from "./Picker";
...
export class CommonPicker extends Component {
  ...
  componentDidMount() {
    if (this.pickerStyle == "cascade") {
      this._getCascadeData(
        this.props.pickerData,
        true,
  ...
  _getStateFromProps(props) {
    ...
    if (
      ...
      Object.keys(pickerData).length == 0 ||
      pickerData.length == 0
    ) {
      pickerData = [[""]];
    }
    ...
    if (pickerStyle === "parallel") {
      ...
      if (!pickerData.some((x) => Array.isArray(x))) {
        ...
        if (pickerData.some((x) => !Array.isArray(x))) {
          console.warn(
            "parallel模式，数组内部要么全部数组，要么全部字符串/数字"
          );
          //如有有，则将非数组全部转换为数组
  ...
  _onValueChange = (pickerValue = this.pickedValue, wheelIndex) => {
    this.props.onValueChange &&
      this.props.onValueChange(
        JSON.parse(JSON.stringify(pickerValue)),
        wheelIndex
      );
  };
  ...
  _getCascadeData = (
    ...
    let wheelSelectedIndexes = Array.from({ length: maxDeepLength });
    //该次数据的实际深度
    ...
    for (let i = 0; i < maxDeepLength; i++) {
      if (wheelIndex > 0 && i < wheelIndex) {
        ...
        if (node.constructor === Array) {
          ...
          //保留原有的值
          node =
            this.state.wheelSelectedIndexes[i] >= 0
              ? node[keys[this.state.wheelSelectedIndexes[i]]]
              : null;
        }
        validDeepLength++;
        continue;
      ...
      if (node.constructor === Array) {
        wheelDatas[i] = [...node];
        if (onInit) {
          wheelSelectedIndexes[i] = node.findIndex(
            (x) => x == pickedValueArray[i]
          );
          ...
          if (i == wheelIndex) {
            ...
            if (
              ...
              this.state.wheelDatas[i][prevSelectedIndex] ==
                wheelDatas[i][prevSelectedIndex]
            ) {
        ...
        wheelDatas[i] = [...keys];
        if (onInit) {
          wheelSelectedIndexes[i] = keys.findIndex(
            (x) => x == pickedValueArray[i]
          );
          if (wheelSelectedIndexes[i] < 0) {
            wheelSelectedIndexes[i] = 0;
        ...
        node =
          wheelSelectedIndexes[i] >= 0
            ? node[keys[wheelSelectedIndexes[i] + ""]]
            : null;
      }
      //更新选中的索引值
    ...
    //计算选中的值
    let returnPickerData = Array.from(
      { length: wheelSelectedIndexes.length },
      () => []
    );
    for (let i = 0; i < wheelSelectedIndexes.length; i++) {
      //wheelDatas可能因为选中的值不对，没有数据
      returnPickerData[i] = wheelDatas[i]
        ? wheelDatas[i][wheelSelectedIndexes[i] + ""]
        : undefined;
    }
    this.setState({
  ...
  _renderCascadeWheel(pickerData) {
    return (
      <View style={[styles.pickerWrap]}>
        {this.state.wheelDatas &&
          (this.state.wheelSelectedIndexes || []).map((x, index) => {
            const wheelStyle = (this.props.wheelStyles || [])[index] || {};
            return (
              <View key={index} style={[styles.pickerWheel, wheelStyle]}>
                <Picker
  ...
  render() {
    return (
      <SafeAreaView
        style={[
          styles.pickerBox,
          { minHeight: 240 + (this.props.showHeader ? 40 : 0) },
          this.props.style,
        ]}
      >
