...
export class DatePicker extends PureComponent {
  static defaultProps = {
    ...
    minDate: dayjs().add(-50, "years").toDate(),
    date: new Date(),
    style: null,
    textColor: "#333",
    textSize: 26,
  ...
  _setDefaultValue = (props = this.props) => {
    //必须要给一个默认值，否则默认为当前时间
    if (!props.date || !dayjs(props.date).isValid()) {
      this.targetDate = dayjs().second(0).toDate();
    } else {
      this.targetDate = props.date;
    }
    //如果超过范围，则默认显示最小值
    if (
      dayjs(this.targetDate).isBefore(props.minDate) ||
      dayjs(this.targetDate).isAfter(props.maxDate)
    ) {
      this.targetDate = dayjs(props.minDate).toDate();
    }
    const { labelUnit, mode } = props;
    let dataArray = [];
    let yearWithUnit = dayjs(this.targetDate).year() + labelUnit.year;
    let monthWithUnit = dayjs(this.targetDate).month() + 1 + labelUnit.month;
    let dateWithUnit = dayjs(this.targetDate).date() + labelUnit.date;
    let hourWithUnit = dayjs(this.targetDate).hour() + labelUnit.hour;
    let minuteWithUnit = dayjs(this.targetDate).minute() + labelUnit.minute;
    switch (props.mode) {
      case "year":
  ...
  _genData = (props = this.props) => {
    if (
      dayjs(props.maxDate).isBefore(dayjs(props.minDate)) ||
      !dayjs(props.minDate).isValid() ||
      !dayjs(props.maxDate).isValid()
    ) {
    ...
    let years = new Set();
    for (
      let i = dayjs(props.minDate).year();
      i <= dayjs(props.maxDate).year();
      i++
    ) {
      years.add(i + props.labelUnit.year);
    }
    let currentYear = dayjs(this.targetDate).year();
    //0-11
    let currentMonth = dayjs(this.targetDate).month();
  ...
  _getMonthesByYear = (props = this.props, currentYear) => {
    //月份(moment取到的月份是0-11)
    let monthes = new Set();
    if (props.mode !== "year" && props.mode !== "time") {
      ...
      //如果跟最小值的年份一样，去除之前的月份
      if (dayjs(props.minDate).year() === currentYear) {
        for (let i = 1; i < dayjs(props.minDate).month() + 1; i++) {
          monthes.delete(i + props.labelUnit.month);
        }
      }
      //如果跟最大值的年份一样，去除之后的月份
      if (dayjs(props.maxDate).year() === currentYear) {
        for (let i = dayjs(props.maxDate).month() + 2; i <= 12; i++) {
          monthes.delete(i + props.labelUnit.month);
        }
      }
    }
  ...
  //currentMonth是0-11
  _getDaysByYearAndMonth = (props = this.props, currentYear, currentMonth) => {
    let days = new Set();
    if (
      props.mode !== "year" &&
      props.mode !== "month" &&
      ...
      if (
        dayjs(props.minDate).year() === currentYear &&
        dayjs(props.minDate).month() === currentMonth
      ) {
        for (let i = 1; i < dayjs(props.minDate).date(); i++) {
          days.delete(i + props.labelUnit.date);
        }
      }
      //如果跟最大值的年份月份一样，去除之后的天数
      if (
        dayjs(props.maxDate).year() === currentYear &&
        dayjs(props.maxDate).month() === currentMonth
      ) {
        //最多是31天
        for (let i = dayjs(props.maxDate).date() + 1; i <= 31; i++) {
          days.delete(i + props.labelUnit.date);
        }
      }
    }
  ...
  _getNewMonthesAndSelectedMonthByYear = (yearWithUnit, monthWithUnit) => {
    ...
    let nextMonthes = this._getMonthesByYear(
      this.props,
  ...
  _getNewDaysAndSelectedDayByYearAndMonth = (
    yearWithUnit,
    monthWithUnit,
    dayWithUnit
  ) => {
    const { mode, labelUnit } = this.props;
    let nextDays = this._getDaysByYearAndMonth(
      this.props,
      parseInt(yearWithUnit.replace(labelUnit.year, "")),
  ...
  //根据数组转换成moment对象
  _dataArrayToMoment = (dataArray) => {
    const { mode, labelUnit } = this.props;
    let date = dayjs(dayjs().format("YYYY-MM-DD"), "YYYY-MM-DD");
    switch (mode) {
      case "year":
        date = date.year(dataArray[0].replace(labelUnit.year, ""));
        break;
      case "month":
        date = date
          .year(dataArray[0].replace(labelUnit.year, ""))
      ...
      case "time":
        date = date
          .hour(dataArray[0].replace(labelUnit.hour, ""))
  ...
  _onDateChange = (value, wheelIndex) => {
    ...
    switch (mode) {
      ...
      case "month":
        ...
        //可能会根据最大最小时间影响月份的数据
        if (wheelIndex == 0) {
          const [monthes, newMonthWithUnit] =
            this._getNewMonthesAndSelectedMonthByYear(value[0], value[1]);
      ...
      case "datetime":
        if (wheelIndex === 0) {
          const [monthes, newMonthWithUnit] =
            this._getNewMonthesAndSelectedMonthByYear(value[0], value[1]);
          const [days, newDayWithUnit] =
            this._getNewDaysAndSelectedDayByYearAndMonth(
              value[0],
              newMonthWithUnit,
              value[2]
            );
      ...
      <CommonPicker
        ...
        onPickerConfirm={() => {
          this.props.onPickerConfirm &&
            this.props.onPickerConfirm(this.targetDate);
        }}
      />
    );
  }
